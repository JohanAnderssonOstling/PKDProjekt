...
commands ["/quit"] ClientState {quit=False,muted=x} = 
    ("disconnect", 
    ClientState {quit=True,muted=x}) --client quits server
commands ["/mute",username] (ClientState {quit=x,muted=mUsers}) = 
    ("Muted " ++ username, 
    ClientState {quit=x, muted=(Map.insert username True mUsers)} ) --mutes specified username
